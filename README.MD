...
Using aura query api on port 443 instead of bolt 
```sh
docker run --rm --name aura-proxy -d -v ./queryapi.conf:/etc/nginx/nginx.conf -p 443:443 aura-proxy:v1
```
...
dscacheutil -q host -a name database.haklof.corp
